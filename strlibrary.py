...
class ReportDB:
    ...
    # Added the follow functions: check_overlap, check_saturation, check_ils_failure,
    # def check_ils_failure(stutter_data, flags):
    #     """The function sees the sample has been flagged with an ILS failure."""
    #     for allele_call in stutter_data:
    #         if (allele_call[Marker] != ''
    #             and allele_call[Marker] != "AMEL") \
    #                 and allele_call[Marker] != 'DYS391':  # this checks for and removes OL which cause issue
    #             if allele_call[Sample_Comments] in ["ILS Failure", "ILS Fails", "Misplating Fails", "Size Call Failed"]:
    #                 flags[allele_call[Marker]]["Use P1"] = False
    #                 flags[allele_call[Marker]]["Use P2"] = False
    #
    # def check_overlap(stutter_data, profiles, flags):
    #     """This function checks for overlap. It determines the separation of
    #     the parent peaks and sets a value indicating that separation for
    #     use in other functions.
    #
    #     0.2 and 0.8 refer to halfback stutter to simplify the math."""
    #
    #     """
    #     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    #     may need to change this so that it uses an allele call,
    #     this could be calculated based on the overlap
    #     e.g. 15, 14 could include 13 as a peak that won't count?
    #     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"""
    #
    #     sample_for_data = stutter_data[1][1].split("_")[1]
    #     profile = profile_for_sample(sample_for_data, profiles)
    #     for key in profile:
    #         if len(profile[key]) == 1:
    #             continue
    #         elif key in ['AMEL', 'DYS391']:
    #             continue
    #         else:
    #             for allele1 in profile[key]:
    #                 for allele2 in profile[key]:
    #                     if (Decimal(allele2) - Decimal(allele1)) == Decimal("1"):
    #                         flags[key]['Overlapping'] = 1
    #                         flags[key]["Use P1"] = False
    #                         flags[key]["Use P2"] = False
    #                     elif (Decimal(allele2) - Decimal(allele1)) == Decimal("2"):
    #                         flags[key]['Overlapping'] = 2
    #                         flags[key]["Use P1"] = False
    #                     elif (Decimal(allele2) - Decimal(allele1)) == Decimal("0.2") \
    #                             or (Decimal(allele2) - Decimal(allele1)) == Decimal("0.8"):
    #                         flags[key]['Overlapping'] = 0.5
    #                         flags[key]["Use P1"] = False
    #                     elif (Decimal(allele2) - Decimal(allele1)) == Decimal("1.2") \
    #                             or (Decimal(allele2) - Decimal(allele1)) == Decimal("1.8"):
    #                         flags[key]["Overlapping"] = 1.5
    #
    #                     elif (Decimal(allele2) - Decimal(allele1)) == Decimal("3"):
    #                         flags[key]["Overlapping"] = 3
    #
    #
    # def check_saturation(stutter_data, profiles, flags):
    #     """The function has multiple roles [refactor?].
    #     It sets the Peak Count field in the flags dictionary.
    #     It sets the peak base pair sizes for all parent peaks in the
    #     current locus.
    #     It checks for stutter and sets the appropriate flags."""
    #     sample_for_data = stutter_data[1][1].split("_")[1]
    #     profile = profile_for_sample(sample_for_data, profiles)
    #     for key in profile:
    #         if key in ["Sample Name", "AMEL", "DYS391"]:
    #             continue
    #         else:
    #             flags[key]["Peak Count"] = len(profile[key])
    #             flags[key]["Profile Peaks"] = profile[key]
    #             flags[key]["Stutter to Call"] = determine_stutter(flags[key]["Profile Peaks"])
    #     for allele_call in stutter_data:
    #         if allele_call[Marker] != '':
    #             if allele_call[Allele] in profile[allele_call[Marker]]:
    #                 flags[allele_call[Marker]]["Peak BP"].append(allele_call[Size])
    #
    #                 continue
    #             else:
    #                 continue
    #     for allele_call in stutter_data:
    #         if int(allele_call[Height]) >= 30000 and allele_call[Marker] != '':
    #             flags[allele_call[Marker]]['Saturation'] = True
    #             flags[allele_call[Marker]]["Use P1"] = False
    #             flags[allele_call[Marker]]["Use P2"] = False
